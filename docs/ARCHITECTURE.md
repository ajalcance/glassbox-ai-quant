...
| 4 | **Risk gate** | Pure function, 17 ordered checks: kill switch, market window, reconciliation clean, defined-risk assert per leg, max loss cap, heat cap, Greeks bands, concentration, correlation, liquidity, daily loss, rate caps, duplicates, **session room** — an intraday thesis needs half its horizon before the close or it cannot resolve — and a **corporate-action blackout** — no new position in a name with a dividend, split, merger or spinoff inside the horizon. Every result logged, passes included. |
...
| 6 | **Trade management** | An intraday horizon is truncated to the closing bell, so a thesis that expired at the close is never carried overnight unmanaged. Triple barrier: 50% max-profit take (credit) / 2× stop / time exit; break-even at 60% of target; ATR trail on directional; never hold within 24h of expiry; flatten all Thu Sep 3 close. Alpaca has no bracket/OTO on mleg — this layer IS our bracket. |
