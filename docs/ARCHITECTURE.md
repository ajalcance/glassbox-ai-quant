...
| 3 | **Sizing** | R = 1.0% × meta-label multiplier (p<0.55 → none; 0.55–0.85 → 0.4–1.0× linear) ∧ vol-target overlay; take the smaller. Caps 1.5%/position, 3%/underlying. |
| 4 | **Risk gate** | Pure function, 19 ordered checks: kill switch, market window, reconciliation clean, defined-risk assert per leg, max loss cap, heat cap, Greeks bands, concentration, correlation, liquidity, daily loss, rate caps, duplicates, **session room** — an intraday thesis needs half its horizon before the close or it cannot resolve — a **flatten deadline** — never open a position the manager would close on its next tick — a **macro blackout** — no new short premium into a scheduled release, whose event premium is exactly what makes the edge test misread — and a **corporate-action blackout** — no new position in a name with a dividend, split, merger or spinoff inside the horizon. Every result logged, passes included. |
...
| 6 | **Trade management** | **Size never changes once opened** — binary hold or close. Scaling out is unavailable at one-contract positions; scaling in is declined as adding risk to an open thesis. Risk is reduced through exits instead. An intraday horizon is truncated to five minutes before the closing bell, so the time barrier is reachable while the manager is still running; whether an expired position may then carry overnight is the bell gate's decision, not a blanket rule. Eleven severity-ordered barriers, obligation before protection before profit-taking: deadline, expiry risk, macro risk, thesis complete, thesis broken, stop (1.5× credit / 40% of debit), the bell gate (may this carry overnight?), trailing lock (arms at 60% of target, keeps 50% of peak), break-even, profit (50% of credit / 100% of debit), and time — which banks winners only. Positions are marked at liquidation value, never mid. Alpaca has no bracket/OTO on mleg — this layer IS our bracket. |
