...
| 4 | **Risk gate** | Pure function, 16 ordered checks: kill switch, market window, reconciliation clean, defined-risk assert per leg, max loss cap, heat cap, Greeks bands, concentration, correlation, liquidity, daily loss, rate caps, duplicates, and a **corporate-action blackout** — no new position in a name with a dividend, split, merger or spinoff inside the horizon. Every result logged, passes included. |
